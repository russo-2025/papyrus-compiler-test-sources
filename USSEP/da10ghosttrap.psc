...
; 		debug.trace(self + "has been activated")
		;USKP 2.0.1 - Objects involved here will throw errors if the player has picked them up first.
        objSelf = self.getReference()
        if !hauntingOver && akActivator == objSelf && !objSelf.IsDeleted()
            Cell parentCell = objSelf.GetParentCell()
            If parentCell && parentCell == game.GetPlayer().GetParentCell() && objSelf.Is3DLoaded()
                fireTrap()
            endIf
        endif
	endEvent
