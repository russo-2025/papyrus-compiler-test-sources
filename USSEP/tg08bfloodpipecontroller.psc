...
Function TG08EnableLinkChain(ObjectReference CurrentLink)
	...
	;USKP 2.1.3b - Passing a NONE is normal so just bail out if one comes in cause nothing can be done with it.
	if( Currentlink == None )
		Return
	EndIf
...
; 		debug.trace(self + ": Current enable link is " + CurrentLink)
	...
	;USKP 2.0.4 - For the love of Talos, if this value is bogus, don't sit here spinning forever filling up the logs with garbage!
	if( LastLink )
