...
; 			;Debug.Trace("Activating completion ref.")
			;USKP 2.0.4 - No sanity check.
			if( refActivateOnComplete )
...
; 	;Debug.Trace("Activating next enemy.")
	;USKP 2.1.3 Bug #14369
	ObjectReference BMTemp = BattleManager.GetNthLinkedRef(currentEnemyLink).GetLinkedRef(EnemyLinkKeyword)
	if( BMTemp != None )
...
; 	;Debug.Trace(BattleManager.GetNthLinkedRef(currentEnemyLink - 1).GetLinkedRef(EnemyLinkKeyword) + " " + (BattleManager.GetNthLinkedRef(currentEnemyLink - 1).GetLinkedRef(EnemyLinkKeyword) As Actor).IsDead())
	;USKP 2.1.3 Bug #14369
	Actor BMTemp2 = (BattleManager.GetNthLinkedRef(currentEnemyLink - 1).GetLinkedRef(EnemyLinkKeyword) As Actor)
	if( BMTemp2 != None )
		if( BMTemp2.IsDead() && currentEnemyLink < totalEnemies)
			ActivateNextEnemy()
		EndIf
	EndIf
EndFunction
