	...
	if (pInCoffin == 0)
		if (db04.GetStage() == 10)
			;if (Game.GetPlayer().HasLOS(HeadAlias.GetReference()))
				...
				USKPNazirAliasDB04.GetActorReference().StopCombat()
				(Game.GetPlayer() as Actor).StopCombatAlarm()
				pCiceroAlias.GetReference().Moveto(pCiceroOutChamberMarker)
...
if (akActionRef == Game.GetPlayer())
	...
	If(DB10.GetStage() == 50)
		if pNazirAlias.GetActorReference().IsInCombat() == 0
			if (pCoffinEscape == 0)
			;if (Game.GetPlayer().HasLOS(HeadAlias.GetReference()))
				;Game.GetPlayer().TranslateToRef(TranslateMarker, 100, 100)
				Game.DisablePlayerControls(ablooking = true, abCamSwitch = true)
				...
				pBabetteAlias.TryToEnable()
				NightMotherCoffinWater.Enable()
				NightMotherCorpseWater.Enable()
				pNazirAlias.TryToMoveto(NazirLakeMarker)
				pBabetteAlias.TryToMoveto(BabetteLakeMarker)
				DB10.SetStage(55)
				DBSanc_NMCoffinLightToggle.Disable()
				Utility.Wait (10)
...
ReferenceAlias Property USKPNazirAliasDB04 Auto
