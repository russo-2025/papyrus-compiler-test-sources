...
;UDBP 2.0.3 - Reversed the calls on the MoveTo and kill commands. Moving dead bodies isn't reliable.
Alias_Varona.GetActorReference().GetActorBase().SetEssential(false)
Alias_Varona.GetReference().MoveTo(Alias_VaronaDeadMarker.GetReference())
utility.wait(1)
Alias_Varona.GetActorReference().Kill()

Alias_AshSpawn1.GetReference().Enable()
Alias_AshSpawn2.GetReference().Enable()
Alias_AshSpawn3.GetReference().Enable()
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Neloth.GetActorReference().AddToFaction(DLC2StaffMakerFaction)
Alias_Talvas.GetActorReference().AddToFaction(DLC2StaffMakerFaction)

Alias_Drovas.GetReference().AddItem(DLC2TelMithrynStewardKey, 1)

alias_Neloth.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 1)
alias_Drovas.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 1)

DLC2StaffEnchanterNavmeshControl.Disable()
Alias_EnchanterDoor.GetReference().Lock(false)
