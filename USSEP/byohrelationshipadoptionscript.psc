...
Function UHFP_ReassignBedOwnerships( int Destination )
	...
	if( Destination <= 5 || Destination > 8 ) ;Vanilla house, or possibly a mod added house. Strip all ownership from HF single beds since it doesn't matter in this case.
	...
	elseif( Destination == 6 ) ;Falkreath
...
Function AdoptChild(Actor childToAdopt)
	...
	;USSEP 4.3.6 Bug #35669.
	if ((childDestination <= 5 || CCHouse.IsCCHouseDestination(childDestination)) && childDestinationLoc.IsSameLocation(childCurrentLoc, LocTypeCity))
	...
	if (!BYOHRelationshipAdoptionNewAdoptionHandler.IsRunning())
		BYOHAdoptionNewAdoptionEvent.SendStoryEvent(TranslateHouseIntToInteriorLoc(childDestination), childToAdopt) ;USSEP 4.3.6 Bug #35669. Don't send a NONE. You're sending the child to this location so it NEEDS to be valid.
...
Function MoveFamily()
	...
	if (Spouse.GetActorRef() != None && !Spouse.GetActorRef().IsDead())
		if (!Spouse.GetActorRef().IsInFaction(CurrentFollowerFaction)) ;USSEP 4.3.6 Bug #35669.
	...
	EndIf
	if (Child1.GetActorRef() != None) ;USSEP 4.3.6 Bug #35669.
		Child1.GetActorRef().MoveTo(newHomeRef)
		Child1.GetActorRef().EvaluatePackage()
	EndIf
	if (Child2.GetActorRef() != None) ;USSEP 4.3.6 Bug #35669.
		Child2.GetActorRef().MoveTo(newHomeRef)
		Child2.GetActorRef().EvaluatePackage()
	EndIf
	if (FamilyPet.GetActorRef() != None && !FamilyPet.GetActorRef().IsInFaction(CurrentFollowerFaction)) ;USSEP 4.3.6 Bug #35669.
...
Function DirtyMoveFamily()
	...
	;the Scheduler can't start unless Child1 is in the house, and by definition, the player is there, so we
...
Function PlayerLocationChanged(Location newLoc, Location oldLoc)
	...
	if (moveQueued)
		...
		;UHFP 2.0.1 - This throws all manner of errors if one does not first check to see if player is in a None.
		;Player in None means they're not in the city anymore. So you can go ahead and do your off screen move now.
		if( newLoc == None )
			MoveFamily()
		elseif (newLoc != Child1.GetActorReference().GetCurrentLocation() && \
			(Spouse.GetActorReference() == None || Spouse.GetActorReference().IsInFaction(CurrentFollowerFaction) || newLoc != Spouse.GetActorReference().GetCurrentLocation()) && \
			(Child2.GetActorReference() == None || newLoc != Child2.GetActorReference().GetCurrentLocation()))
			;Debug.Trace("Children are not in the player's new location.")
			if (!Child1.GetActorReference().GetCurrentLocation().IsChild(newLoc) && \
				(Spouse.GetActorReference() == None || Spouse.GetActorReference().IsInFaction(CurrentFollowerFaction) || !Spouse.GetActorReference().GetCurrentLocation().IsChild(newLoc)) && \
				(Child2.GetActorReference() == None || !Child2.GetActorReference().GetCurrentLocation().IsChild(newLoc)))
				;Debug.Trace("Children are not in the parent of the player's new location.")
				;USSEP 4.3.6 Bug #35669.
				if ((newLoc == TranslateHouseIntToLoc(newHome) && (!oldLoc || !newLoc.IsChild(oldLoc))) || \
					(!newLoc.IsChild(Child1.GetActorReference().GetCurrentLocation()) && \
					(Spouse.GetActorReference() == None || Spouse.GetActorReference().IsInFaction(CurrentFollowerFaction) || !newLoc.IsChild(Spouse.GetActorReference().GetCurrentLocation())) && \
					(Child2.GetActorReference() == None || !newLoc.IsChild(Child2.GetActorReference().GetCurrentLocation()))))
	...
	if ((newLoc == CurrentHomeHouse.GetLocation() || newLoc == CurrentHomeExterior.GetLocation()) && \
		...
		if (Utility.GetCurrentGameTime() - playerLastSeen > WelcomeHomeDelay)
			...
			if (GiftStoredValueChild1 > 0)
				...
				Child1.GetActorReference().AddItem(Gold001, GiftStoredValueChild1)
				GiftStoredValueChild1 = 0
			EndIf
			if (GiftStoredValueChild2 > 0)
				;Debug.Trace("Adding " + GiftStoredValueChild2 + " stored gold to Child 2.")
				Child2.GetActorReference().AddItem(Gold001, GiftStoredValueChild2)
