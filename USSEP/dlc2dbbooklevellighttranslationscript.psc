...
; 			debug.Trace("Starting Light Translation")
		...
		if (self.IsDisabled() == FALSE)
			;USLEEP 3.0.8 Bug #14339: Added check to make sure that the cell is still attached (the vanilla script checks this for the first transition, but
			;forgot to check it for all subsequent transitions too):
			if bPlayerCurrentlyInCell
				;USLEEP 3.0.8 Bug #14339: Added checks to make sure that the target ref is in a currently loaded cell:
				objectReference currentLinkTargetRef = GetNthLinkedRef(iCurrentLinkTarget)
				if CurrentLinkTargetRef && CurrentLinkTargetRef.GetParentCell() && CurrentLinkTargetRef.GetParentCell().IsAttached()
					self.TranslateToRef(currentLinkTargetRef, fMovementSpeed, 0)
				endif
			endif
		endif
	endif
