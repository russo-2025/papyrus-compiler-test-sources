...
if DLC2TameDragon.dragonAlias.GetReference() || DLC2TameDragon.dragonReleasedAlias.GetReference()
...
Alias_FerryDragon.GetActorReference().Enable()
SetObjectiveCompleted(25)
SetObjectiveDisplayed(30) ; tame dragon
; make sure dragon is set to land on near side
utility.wait(0.5)
Alias_FerryDragon.GetActorReference().SetForcedLandingMarker(NearLandingMarker)
...
; enable dragons
Alias_Dragon2.GetReference().Enable()
Alias_Dragon3.GetReference().Enable()
; disable Miraak's dragon for now
Alias_FerryDragon.GetReference().Disable()
Alias_Lurker.GetReference().Disable()
;END CODE
EndFunction
;END FRAGMENT
...
Alias_FerryDragon.GetActorReference().ClearForcedLandingMarker() ; dragon now lands normally
Alias_FerryDragon.GetActorReference().IgnoreFriendlyHits(false)
; clean up MQ05
...
if DLC2MH02.GetStageDone(35) == True
                ; delete them
                Alias_ThirskWorker1.GetReference().Delete()
                Alias_ThirskWorker1.GetReference().Delete()
...
;UDBP 2.0.2 - Cultist road encounters may still be running. Shut those down.
...
;BEGIN CODE
;fight Starts
Alias_Miraak.GetActorReference().RemoveFromFaction(DLC2MQ06NonHostileFaction)
Alias_Dragon2.GetActorReference().RemoveFromFaction(DLC2MQ06NonHostileFaction)
Alias_Dragon3.GetActorReference().RemoveFromFaction(DLC2MQ06NonHostileFaction)
DLC2MQ06HostileFaction.setEnemy(PlayerFaction)
DLC2MQ06MiraakFaction.SetEnemy(PlayerFaction)
Alias_Miraak.GetActorReference().SetAv("Aggression",1)
Alias_Dragon2.GetActorReference().SetAv("Aggression",1)
Alias_Dragon3.GetActorReference().SetAv("Aggression", 1)
MiraakBattleAlias.InitializeMiraakBattle() ; UDBP 2.0.2 - Relying on his combat state alone to start this doesn't seem to work.
alias_Miraak.GetActorReference().startCombat(game.GetPlayer())
Alias_Dragon2.GetActorReference().startCombat(game.GetPlayer())
Alias_Dragon3.GetActorReference().startCombat(game.GetPlayer())
alias_FerryDragon.GetActorReference().getActorBase().setEssential(False)
;alias_FerryDragon.GetActorReference().RemoveFromFaction(DLC2ApocryphaFaction)
...
Alias_Miraak.GetActorReference().SetRestrained(true)
;Alias_Miraak.GetActorReference().StopCombatAlarm()
; add armor
;Alias_Miraak.GetActorReference().EquipItem(DLC2HMKillsMiraak)
;END CODE
EndFunction
;END FRAGMENT
...
alias_Miraak.GetActorReference().StopCombat()
alias_Miraak.GetActorReference().EvaluatePackage()
;END CODE
EndFunction
;END FRAGMENT
...
Alias_FerryDragon.GetActorReference().IgnoreFriendlyHits(true)
; make sure dragon is set to land on near side
Alias_FerryDragon.GetActorReference().SetForcedLandingMarker(NearLandingMarker)
...
Alias_Miraak.GetActorReference().SetAv("Aggression", 0)
Alias_Dragon2.GetActorReference().SetAv("Aggression", 0)
Alias_Dragon3.GetActorReference().SetAv("Aggression", 0)
;END CODE
EndFunction
;END FRAGMENT
...
Alias_FerryDragon.GetActorReference().SetForcedLandingMarker(TempleLandingMarker)
...
;Next 2 added by UDBP 2.0.2.
DLC2MQ06MiraakBossBattle Property MiraakBattleAlias Auto
