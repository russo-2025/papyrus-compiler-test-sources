Scriptname SolitudeOpeningSetStageScript extends ReferenceAlias  
...
int property prereqStageMinOPT = -1 auto
{ OPTIONAL: stage that must be > than this for this trigger to fire }

int property prereqStageMaxOPT = -1 auto
{ OPTIONAL: stage that must be < than this trigger to fire }
...
; 	debug.trace(self + " testForTrigger, triggerRef=" + triggerRef + ", triggerType=" + pTestTriggerType)
	if TriggerType == pTestTriggerType
		if triggerRef == getPlayer()
			if GetOwningQuest().getStageDone(prereqStageMinOPT) == 1 && GetOwningQuest().getStageDone(prereqStageMaxOPT) == 0 
