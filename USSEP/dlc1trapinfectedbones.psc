...
; 		;debug.Trace("Sound trap has fired")
		...
		;UDGP 2.0.2 - Moved all statements dealing with myTarget inside the if/endif block.
		actor myTarget
		if (lastTriggerRef as actor)
			...
			CreateDetectionEvent(myTarget, soundLevel) ; creates a detection event, 3 = generic event
		EndIf
