...
;BEGIN CODE
setObjectiveDisplayed(15) ; Ask Falk Firebeard about Wolfskull Cave. [USKP 1.3.0 - wrong objective number fixed]
;END CODE
EndFunction
;END FRAGMENT
...
;USKP 2.0.1 - No sanity checks.
if( !( alias_MS06Cultist1.GetActorReference().IsDead() ) )
  alias_MS06Cultist1.GetActorReference().StartCombat(Game.GetPlayer())
EndIf
if( !( alias_MS06Cultist2.GetActorReference().IsDead() ) )
  alias_MS06Cultist2.GetActorReference().StartCombat(Game.GetPlayer())
EndIf
;END CODE
EndFunction
;END FRAGMENT
...
Function Fragment_3()
;BEGIN CODE
setObjectiveCompleted(15)      ;Ask Falk Firebeard about Wolfskull Caverns. [USKP 1.3.0 - wrong objective number fixed]
...
;EndIf
Alias_MS06RitualEnabler.GetRef().Disable() ;USKP 1.1
int DayV = GameDaysPassed.GetValueInt() ;USKP 1.3.3
USLEEPMS06StartDay.SetValue(DayV + 7) ;USKP 1.3.3
;END CODE
EndFunction
;END FRAGMENT
...
;USKP 2.0.3 Bug #14869 - Dead actors, even necromancers, cannot attack
if( !alias_MS06HeadNecromancer.GetActorReference().IsDead() )
    alias_MS06HeadNecromancer.GetActorRef().EvaluatePackage()
	alias_MS06HeadNecromancer.GetActorReference().StartCombat(Game.GetPlayer())
EndIf
;END CODE
EndFunction
;END FRAGMENT
...
GlobalVariable Property USLEEPMS06StartDay Auto ;USKP 1.3.3
GlobalVariable Property GameDaysPassed Auto ;USKP 1.3.3
