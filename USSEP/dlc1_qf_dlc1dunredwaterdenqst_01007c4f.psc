...
Function KnockoutPlayer()
	...
	;UDGP 2.0.2 - Sanity check, yours weren't quite enough.
	;UDGP 2.0.7 - If your follower has been told to wait somewhere, don't bring them along for this ride.
	if PlayerFollower.GetReference()
		if(PlayerFollower.GetActorReference().GetAV("WaitingForPlayer") == 0)
			PlayerFollower.GetActorReference().MoveTo(game.getPlayer())
		endif
	endif
	if PlayerAnimalFollower.GetReference()
		if(PlayerAnimalFollower.GetActorReference().GetAV("WaitingForPlayer") == 0)
