...
Event OnCellAttach()
	...
	if (bcs != None)
		...
		if (shouldBeDisabled && !Self.GetLinkedRef(LinkCustom01).IsDisabled())
			MoveBooks(bcs)
		EndIf
	;~	Debug.Trace(Self + "Bookshelf processed. Object IsDisabled() was: " + shouldBeDisabled)
	Else
		...
		if (wras != None)
			...
			bool shouldBeDisabled = Self.GetLinkedRef(WRackTrigger).IsDisabled(); UHFP 2.0.1 - Whether or not to disable it DOES need to be determined first!
			if (shouldBeDisabled)
				Self.Disable()
			...
			Else
				...
				EndIf
			EndIf
			;Debug.Trace(Self + "Weapon Rack processed. Object IsDisabled() was: " + shouldBeDisabled)
		Else
			;That's all the prefabs we need to disable, at least for now.
			;Debug.Trace(Self + "Unidentified Prefab.")
		EndIf
	EndIf		
EndEvent

Function MoveBooks(PlayerBookshelfContainerScript bcs)
	;USSEP 4.1.5 Bug #13862
	bcs.RemoveAllItems( Self.GetLinkedRef(LinkCustom01) )
	Utility.Wait(1)
	bcs.USSEP_ClearAllBookForms()
	;Everything below this point has been disabled because it's unconditionally moving the items to the chest anyway. It's much faster and less prone to errors to just do this with thse 2 function calls.
