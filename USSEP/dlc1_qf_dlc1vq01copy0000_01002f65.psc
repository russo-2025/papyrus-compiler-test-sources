...
Alias_GargoyleAmbush.GetActorReference().CreateDetectionEvent(Alias_GargoyleAmbush.GetActorReference(), 100)
Alias_Serana.GetActorReference().StartCombat(Alias_GargoyleAmbush.GetActorReference())
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Agmaer.GetActorReference().SetOutfit(DLC1OutfitAgmaer)
Alias_Agmaer.GetActorReference().AddItem(DLC1DawnguardAxe, 1)
...
Alias_Serana.TryToMoveTo(DLC1VQ02SeranaStart)
...
Alias_Serana.TryToStopCombat()
Alias_Serana.GetActorReference().StopCombatAlarm()
DLC1VampireCastleGuardRef.StopCombat()
DLC1VampireCastleGuardRef.StopCombatAlarm()
Game.GetPlayer().StopCombat()
Game.GetPlayer().StopCombatAlarm()

;Call EVP on Serana
Alias_Serana.TryToEvaluatePackage()
...
Alias_Harkon.GetActorReference().PlayIdle(OffsetCarryMQ201DrinkR)
;END CODE
EndFunction
;END FRAGMENT
...
;UDGP 1.2.4 - Move follower to exterior map marker instead of to the player.
;Script delay could cause them to get moved inside the castle otherwise.
if Follower
	Follower.TryToMoveTo(CastleMapMarker)
endif
if FollowerAnimal
	FollowerAnimal.TryToMoveTo(CastleMapMarker)
endif
SetStage(200)
;END CODE
EndFunction
...
Actor Property DLC1VampireCastleGuardRef  Auto  
