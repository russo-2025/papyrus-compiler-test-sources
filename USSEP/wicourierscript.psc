...
function GiveItemsToPlayer()
	pWICourierItemCount.SetValue(0)
 	...
 	;USSEP 4.3.2 Bug #34138 - This segment has been revised because CC and VC content have used the courier but supplied no delivery dialogue for them and the notification of "something" being received is better than nothing.
	;If SKSE is in use it will only show the generic message if nothing more is present in the courier's chest.
	int skseversion = SKSE.GetVersion()

	if( skseversion >= 2 )
		if( pCourierContainer.GetNumItems() > 0 )
	...
	; USSEP 4.2.9 Bug #32768
	RegisterForSingleLOSLost( Game.GetPlayer(), pCourier )
EndFunction

; USSEP 4.2.9 Bug #32768 - Cleaner event for Courier transfer.
Event OnLostLOS( Actor akViewer, ObjectReference akTarget )
	Stop()
EndEvent

;Old update event. This needs to remain to unregister itself.
Event OnUpdate()		;registered in stage 0, unregistered in stage 200
	;Modified by USKP 1.2.7 for easier checking of NONE locations.
	Location CourierLoc = pCourier.getCurrentLocation()
	Location PlayerLoc = Game.GetPlayer().getCurrentLocation()

	UnRegisterForUpdate() ; USSEP 4.2.9 Bug #32768
	if( !Game.GetPlayer().HasLOS( pCourier ) )
		Stop()
	else
		RegisterForSingleLOSLost( Game.GetPlayer(), pCourier )
		return
	...
	; Also going to try and short circuit the courier or player being in a NONE so it doesn't break whatever was supposed to happen here.
	; USKP 1.3.3 rearranged this. It was still spamming, so had to sanity check the Courier AND the player.
	if( CourierLoc != None && PlayerLoc != None )
		if( CourierLoc.IsSameLocation(PlayerLoc, pLocTypeHabitation) )
