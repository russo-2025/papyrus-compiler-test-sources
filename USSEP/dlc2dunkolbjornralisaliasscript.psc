...
; 	;Debug.Trace("Ralis updates. We're waiting for the player to leave...")
	;UDBP 2.0.3 - Ralis doesn't stick around Kolbjorn and can end up too close to the player for this check to succeed, especially if taken as a follower before the quest is done processing.
	;Use the static marker that's already defined as a property instead.
	if (player.GetDistance(DLC2dunKolbjorn_PlayerOutsideKolbjornMarker) < 8500 || player.GetCurrentLocation() == DLC2KolbjornBarrowLocation)
...
; 		Debug.Trace("Advancing Kolbjorn exterior.")
		...
		ElseIf (DLC2dunKolbjornQST.IsStageDone(550) && !DLC2dunKolbjornQST.IsStageDone(600))
