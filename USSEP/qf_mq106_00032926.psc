...
Alias_Dragon.GetActorReference().setActorValue("aggression", 1)
...
Alias_dragon.GetActorReference().SetForcedLandingMarker(DragonLandingMarker)
...
Alias_Delphine.GetReference().MovetoIfUnloaded(KynesgroveMoveMarker)
LeaveRiverwoodScene.Stop()
KynesgroveScene.Start()
Alias_Alduin.TryToEvaluatePackage()
AlduinWeather.SetActive(true, true)
; remove Delphine's torch
Alias_Delphine.GetReference().RemoveItem(Torch01, 99)
;END CODE
EndFunction
;END FRAGMENT
...
RegisterForSingleLOSGain(Alias_Player.GetActorReference(), Alias_Alduin.GetActorReference())
; make sure all dead
Alias_KynesgroveGuard.TryToKill()
Alias_KynesgroveGuard2.TryToKill()
Alias_KynesgroveGuard3.TryToKill()
Alias_KynesgroveGuard1Disabled.TryToKill()
Alias_KynesgroveGuard2Disabled.TryToKill()
Alias_KynesgroveGuard3Disabled.TryToKill()
; make guards temporarily unaggressive
;USKP 2.0.2 - Sanity checks. These aliases could be empty if the guards were already dead.
if( Alias_KynesgroveGuard.GetActorReference() )
	Alias_KynesgroveGuard.GetActorReference().SetActorValue("aggression", 0)
EndIf
if( Alias_KynesgroveGuard2.GetActorReference() )
	Alias_KynesgroveGuard2.GetActorReference().SetActorValue("aggression", 0)
EndIf
if( Alias_KynesgroveGuard3.GetActorReference() )
	Alias_KynesgroveGuard3.GetActorReference().SetActorValue("aggression", 0)
EndIf
;END CODE
EndFunction
;END FRAGMENT
...
Actor delphine = Alias_Delphine.GetActorReference()
delphine.SetOutfit(DelphineOutfit)
;USKP 2.1.3 Bug #19383
...
;USKP 2.0.2 - Sanity checks. These aliases could be empty if the guards were already dead.
if( Alias_KynesgroveGuard.GetActorReference() )
	Alias_KynesgroveGuard.GetActorReference().SetActorValue("aggression", 1)
EndIf
if( Alias_KynesgroveGuard2.GetActorReference() )
	Alias_KynesgroveGuard2.GetActorReference().SetActorValue("aggression", 1)
EndIf
if( Alias_KynesgroveGuard3.GetActorReference() )
	Alias_KynesgroveGuard3.GetActorReference().SetActorValue("aggression", 1)
EndIf
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Delphine.GetActorReference().AddItem(Torch01, 1) ; give her back her torch
Alias_Delphine.GetActorReference().EvaluatePackage()
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Orgnar.TryToAddToFaction(RentRoomFaction)
Alias_Delphine.TryToRemoveFromFaction(RentRoomFaction)
Alias_Delphine.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 2) ; make confidant
...
Alias_Delphine.GetReference().AddItem(MQDelphineSecretDoorKey, 1)
