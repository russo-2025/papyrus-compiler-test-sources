...
Alias_Odahviing.GetActorReference().StopCombatAlarm()
Alias_Odahviing.GetActorReference().EvaluatePackage()
Alias_Odahviing.GetActorReference().SetRestrained(true)
Alias_Odahviing.GetActorReference().GetLeveledActorBase().SetInvulnerable(true)
...
Alias_Jarl.GetActorReference().RemoveItem(JarlBow, 1)
Alias_Jarl.GetActorReference().RemoveItem(JarlArrow, 12)
Alias_Odahviing.GetActorReference().StopCombatAlarm()
...
Alias_Odahviing.GetActorReference().ClearForcedLandingMarker()
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Odahviing.GetActorReference().SetAllowFlying(false)
Alias_Odahviing.GetActorReference().StopCombatAlarm()
; get him to revive if in bleedout
Alias_Soldier1.GetActorReference().StopCombat()
; enable corpse
if Alias_Soldier1.GetActorReference().GetActorBase() == ImperialSoldier
...
Function Fragment_0()
;BEGIN CODE
if Alias_Paarthurnax.GetActorReference().IsDead() == 0
...
;END AUTOCAST
;BEGIN CODE
;USKP 2.0.4 - Force follower alias if it's filled.
if( DialogueFollower.GetReference() )
	Alias_Follower.ForceRefTo(DialogueFollower.GetReference())
	Alias_Follower.TryToEvaluatePackage()
EndIf
...
if Alias_Jarl.GetActorReference().IsInFaction(GovImperial) == 1
	; create Whiterun soldiers
	Alias_Soldier1.ForceRefTo(Alias_Soldier1StartMarker.GetReference().PlaceAtMe(ImperialSoldier))
	Alias_Soldier2.ForceRefTo(Alias_Soldier2StartMarker.GetReference().PlaceAtMe(ImperialSoldier))
	Alias_Soldier3.ForceRefTo(Alias_Soldier3StartMarker.GetReference().PlaceAtMe(ImperialSoldier))
else
	; create stormcloaks
	Alias_Soldier1.ForceRefTo(Alias_Soldier1StartMarker.GetReference().PlaceAtMe(SonsSoldier))
	Alias_Soldier2.ForceRefTo(Alias_Soldier2StartMarker.GetReference().PlaceAtMe(SonsSoldier))
	Alias_Soldier3.ForceRefTo(Alias_Soldier3StartMarker.GetReference().PlaceAtMe(SonsSoldier))
endif
...
Alias_Odahviing.GetActorReference().SetForcedLandingMarker(WhiterunDragonsreachLandingMarker)
; give Jarl ranged weapon
Alias_Jarl.GetActorReference().AddItem(JarlBow)
Alias_Jarl.GetActorReference().AddItem(JarlArrow, 12)
;END CODE
EndFunction
;END FRAGMENT
...
ReferenceAlias Property DialogueFollower Auto
