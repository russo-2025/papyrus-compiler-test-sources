...
; 			Debug.Trace(self + "Calling DLC2SoulSteal.MiraakAppears()")
	...
	;USSEP 4.1.5 Bug #14379: added this line:
	USSEP_RegisterForAnimationEvents (dragonRef)
...
;;	dragonRef.placeAtME(MAGDragonPowerAbsorbExplosion)
	...
	;USKP 2.0.6 - Bug #17080: Disabled. Setting the ghost flag actually allows the dragon's body to be flung considerable distance when hit by magic.
...
;;
	...
	;USSEP 4.1.5: "Game.GetPlayer()" instead of "GetPlayer()":
	if AbsorbActor == Game.GetPlayer()
...
; 	; 		debug.trace(self + " setting MQ104 stage 90")
	...
	;USSEP 4.1.5 Bug #14379: added this line:
	USSEP_UnregisterForAnimationEvents (dragonRef)
...
;;	dragonRef.placeAtME(MAGDragonPowerAbsorbExplosion)
	...
	;wait(1)
	; ghost the dragon so you can't hit him
	;UDBP 2.0.6 - Bug #17080: Disabled. Setting the ghost flag actually allows the dragon's body to be flung considerable distance when hit by magic.
	;dragonRef.SetGhost(true)
...
;-----------------------------------------------------------------
;	Added by USSEP 4.1.5 for Bug #16140
;-----------------------------------------------------------------
...
function USSEP_RegisterForAnimationEvents (actor dragonRef)
	if dragonRef
		RegisterForAnimationEvent (dragonRef, "BloodFade")
		RegisterForAnimationEvent (dragonRef, "SkinFadeOut")
		RegisterForAnimationEvent (dragonRef, "UnderSkinFadeOut")
	endif
endFunction

function USSEP_UnregisterForAnimationEvents (actor dragonRef)
	if dragonRef
		UnregisterForAnimationEvent (dragonRef, "BloodFade")
		UnregisterForAnimationEvent (dragonRef, "SkinFadeOut")
		UnregisterForAnimationEvent (dragonRef, "UnderSkinFadeOut")
	endif
endFunction
