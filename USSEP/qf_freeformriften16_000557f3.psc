...
;BEGIN CODE
;Quest Completed
Game.GetPlayer().RemoveItem(Alias_FFRiften16Sword.GetReference())
Alias_FFRiften16HarraldAlias.GetActorReference().AddItem(Alias_FFRiften16Sword.GetReference())
;USKP 2.0.3 - Only add to NPC, can't equip without 3D for some dumb reason.
;Alias_FFRiften16HarraldAlias.GetActorReference().EquipItem(Alias_FFRiften16Sword.GetReference())
Game.GetPlayer().AddItem(pReward)
Game.GetPlayer().AddItem(pReward)
if( Alias_FFRiften16HarraldAlias.GetActorReference().GetRelationshipRank(Game.GetPlayer()) <= 1 ) ;USSEP 4.3.2 Bug #33886
  Alias_FFRiften16HarraldAlias.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 1)
endif
pThaneCount.Value += 1
pFFRTQS.ThaneCheck()
...
Game.GetPlayer().AddItem(Alias_FFRiften16Sword.GetReference())
;END CODE
EndFunction
;END FRAGMENT
