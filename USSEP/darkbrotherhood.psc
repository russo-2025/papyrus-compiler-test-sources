...
;These two properties added by USKP to halt the registered update event. 
Quest Property DBForever Auto 
bool Property USKPStopUpdates Auto

;USKP 2.0.4 - Trying to see if the player got caught killing Helvard. Better than no method at all. Will be used to reset the Jarl's relationship status.
;This is being done here because this DB mission can screw up the chance of buying Lakeview Manor.
ReferenceAlias Property Helvard Auto
bool Property USKPCombatDetected Auto
Int Property USKPJarlRelationshipRank Auto

Function USKP_WasPlayerDetected()
	if( Game.GetPlayer().IsInCombat() )
		Actor Target = Game.GetPlayer().GetCombatTarget()
		
		if( Target != Helvard.GetActorReference() )
			USKPCombatDetected = True
		EndIf
	EndIf
EndFunction
...
Event OnUpdate()
	if( USKPStopUpdates == false )
		UnregisterForUpdate()
		USKPStopUpdates = True
	EndIf
