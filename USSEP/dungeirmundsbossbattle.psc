...
Function UpdateLoop()
	...
	While(!Self.GetActorRef().IsDead() && dunGeirmundsQST.GetStage() < 10 && Game.GetPlayer().GetCurrentLocation() == GeirmundsHallLocation)
		...
		elseif Duplicate1Actor && Duplicate2Actor
			if (Duplicate1Actor.IsDead() && Duplicate2Actor.IsDead() && Utility.GetCurrentRealTime() - lastDuplicationTime >= 15)
				Duplicate()
			endif
		EndIf
		Utility.Wait(1)
...
Event OnDeath(Actor akKiller)
	...
	bool bContinue
	While !bContinue
		if Duplicate1Actor && Duplicate2Actor
			if Duplicate1Actor.IsEnabled() || Duplicate2Actor.IsEnabled()
				Utility.Wait(1)
			else
				bContinue = true
			endif
