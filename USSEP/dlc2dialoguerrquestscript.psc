...
; ***	Added by Unofficial Dragonborn Patch v1.0.1
	float carryWeight = player.GetActorValue( "CarryWeight" )
	float inventoryWeight = player.GetActorValue( "InventoryWeight" )
	
	; NOTE: +1 avoids potential rounding errors
	float deltaWeight = (inventoryWeight + 1) - carryWeight
	if ( deltaWeight > 0 )
		player.ModActorValue( "CarryWeight", deltaWeight )
	endif
; ***

	if player.GetWorldSpace() == DLC2SolstheimWorld
		; to Skyrim
; 		debug.trace(self + " to Skyrim")
		if iCostFlag == 1
			player.RemoveItem(pGold001, DLC2CostToSail.GetValueInt())
		elseif iCostFlag == 2
			player.RemoveItem(pGold001, DLC2CostToSailx2.GetValueInt())
		endif
...
; 			Debug.Trace("ERROR: Dawnguard loaded, but couldn't find Serana.")
	...
	count = 0
	;UDBP 2.0.4 - If no followers, don't enter the loop because the first pass will produce an error.
	if( __numberOfFollowers > 0 )
...
Function RestoreFollowers()
	int count = 0
	;UDBP 2.0.1 - If no followers, don't enter the loop because the first pass will produce an error.
	if( __numberOfFollowers > 0 )
		while (count < __numberOfFollowers)
			Actor follower = __currentFollowers[count]
	...
	;USSEP 4.2.2 Bug #28454
	__currentFollowers = new Actor[32] ; This avoids a casting error but still clears the __currentFollowers variable
...
Function FirstTimeTravel()
	...
	DLC2RRASGjalundAlias.TryToMoveTo(GjalundOutOfSightMarker)
	int count = 0
