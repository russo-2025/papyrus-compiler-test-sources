...
If Alias_Boss.GetActorReference().IsDead() == 1
  SetStage(20)
EndIf
;END CODE
EndFunction
...
Alias_Perth.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 2)
Alias_Tuthul.GetActorReference().SetRelationshipRank(Game.GetPlayer(), 1)

;Perth will buy ore
Alias_Perth.GetActorReference().AddtoFaction(FavorJobsMineOreFaction)

;Debug.MessageBox("Civil War needs to activate Soljund's Sinkhole Mine's destructible resource object.")

USLEEPDraugrEnableMarker.disable() ; Added by USKP 1.2.5
Stop()
;END CODE
EndFunction
;END FRAGMENT
