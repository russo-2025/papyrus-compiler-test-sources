...
;BEGIN CODE
;USKP 2.0.1 - No sanity checking. Also need to optimize this a bit now.
Actor PlayerRef = Game.GetPlayer()
PlayerRef.removeallitems(DA15Container, 1)
Game.DisablePlayerControls(false, false, false, false, false, true, false, false, 0)
if( PlayerRef.GetEquippedSpell(1) )
  PlayerRef.UnEquipSpell(PlayerRef.GetEquippedSpell(1),1)
EndIf
if( PlayerRef.GetEquippedSpell(0) )
  PlayerRef.UnEquipSpell(PlayerRef.GetEquippedSpell(0), 0)
endif
if( PlayerRef.GetEquippedSpell(2) )
  PlayerRef.UnEquipSpell(PlayerRef.GetEquippedSpell(2),2)
EndIf
;USLEEP 3.0.4 Bug #20481
if( PlayerRef.GetEquippedShout() )
  PlayerRef.UnEquipShout(PlayerRef.GetEquippedShout())
endif

PlayerRef.moveto(alias_DA15BluePalaWingStartMarker.GetReference())
setstage(90)
;END CODE
EndFunction
;END FRAGMENT
...
Alias_Sheogorath.TryToEvaluatePackage()
;END CODE
EndFunction
;END FRAGMENT
...
Game.GetPlayer().MoveTo(Alias_Dervenin.GetReference())
;END CODE
EndFunction
;END FRAGMENT
...
Function Fragment_5()
;BEGIN CODE
Game.GetPlayer().Additem(Alias_Wabbajack.GetReference())
...
Game.Getplayer().additem(Alias_HipBone.GetReference())
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
Game.GetPlayer().AddItem(alias_PelagiusKey.GetReference())
