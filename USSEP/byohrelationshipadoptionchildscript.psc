...
Event OnLocationChange(Location oldLoc, Location newLoc)
	...
	(BYOHRelationshipAdoption as BYOHRelationshipAdoptionScript).ChildLocationChanged(Self.GetActorReference(), newLoc, oldLoc)
EndEvent
...
Function IssueOrder(float order, float time)
	;Debug.Trace("Child: " + Self.GetActorReference() + " received order " + order)
	...
	Self.GetActorReference().SetAV("Variable06", order)
...
Function IssuePlayOrder(float order)
	...
	(BYOHRelationshipAdoption as BYOHRelationshipAdoptionScript).IssueOrderWithDuration(Self.GetActorReference(), 1)
EndFunction
...
Event OnUpdateGameTime()
	...
	if (ordersReceived)
		...
		Self.GetActorReference().SetAV("Variable06", 0)
		Self.GetActorReference().EvaluatePackage()
	EndIf
EndEvent
...
Function SetGiftState(bool giftState)
	...
	if (!giftsBeingGiven && weaponToEquip != None)
		...
		Self.GetActorReference().EquipItem(weaponToEquip)
...
Event OnItemAdded(Form item, int count, ObjectReference ref, ObjectReference source)
	if (giftsBeingGiven)
		...
		if ((item as Weapon) != None)
			...
			if (Self.GetActorReference().GetEquippedWeapon() != None)
				Weapon weaponToUnequip = Self.GetActorReference().GetEquippedWeapon()
				;Debug.Trace("Unequipping A: " + weaponToUnequip)
				Self.GetActorReference().UnEquipItem(weaponToUnequip)
				Self.GetActorReference().RemoveItem(weaponToUnequip, 1, false, CurrentChildsChest.GetReference())
			EndIf
			if (weaponToEquip != None && Self.GetActorReference().GetItemCount(weaponToEquip) > 0)
				if (weaponToEquip == (item as Weapon))
					;Debug.Trace("Unequipping B1: " + weaponToEquip)
					Self.GetActorReference().RemoveItem(weaponToEquip, Self.GetActorReference().GetItemCount(weaponToEquip) - 1, false, CurrentChildsChest.GetReference())
				Else
					;Debug.Trace("Unequipping B2: " + weaponToEquip)
					Self.GetActorReference().RemoveItem(weaponToEquip, Self.GetActorReference().GetItemCount(weaponToEquip), false, CurrentChildsChest.GetReference())
				EndIf
			EndIf
			if (Self.GetActorReference().GetItemCount(item) > 1)
				;Debug.Trace("Removing Excess:  " + weaponToEquip + " " + count)
				Self.GetActorReference().RemoveItem(item, Self.GetActorReference().GetItemCount(item) - 1, false, CurrentChildsChest.GetReference())
			EndIf
			weaponToEquip = (item as Weapon)
			(BYOHRelationshipAdoption as BYOHRelationshipAdoptionScript).UpdateGiftValues(Self.GetActorReference(), value, 0)
		ElseIf ((item as MiscObject) != None)
			...
			if (item.HasKeyword(BYOHAdoptionClothesKeyword))
				...
				if (clothesToEquip != None)
					...
					Self.GetActorReference().RemoveItem(clothesToEquip, 1, false, CurrentChildsChest.GetReference())
				EndIf
				if (Self.GetActorReference().GetItemCount(item) > 1)
		...
		ElseIf ((item as Book) != None)
			...
			Self.GetActorReference().RemoveItem(item, count, false, CurrentChildsChest.GetReference())
			(BYOHRelationshipAdoption as BYOHRelationshipAdoptionScript).UpdateGiftValues(Self.GetActorReference(), value, 0)
		ElseIf ((item as Potion) != None)
			...
			Self.GetActorReference().RemoveItem(item, count)
			(BYOHRelationshipAdoption as BYOHRelationshipAdoptionScript).UpdateGiftValues(Self.GetActorReference(), value, value)
		Else
...
Function RemoveOldClothesIfPresent(Armor clothes, MiscObject maleToken, MiscObject femaleToken)
	if ((Self.GetActorReference().GetItemCount(clothes) > 0 && !Self.GetActorReference().IsEquipped(clothes)) || Self.GetActorReference().GetItemCount(clothes) > 1)
		;Self.GetActorReference().RemoveItem(clothes) UHFP 2.1.3 -- commented out; not necessary, SetOutfit() already does this.
...
Function EquipNewClothes()
	...
	if (clothesToRemove != None)
		Self.GetActorReference().UnequipItem(clothesToRemove)
		clothesToRemove = None
	EndIf
	;UHFP 2.1.3 -- corrected all of these to set the child's outfit rather than their sleep outfit; commented out EquipItem() calls, as SetOutfit() already equips the new outfit.
	if (clothesToEquip == ClothesChild01Male || clothesToEquip == ClothesChild01Female)
		clothesToRemove = ClothesChild01
		Self.GetActorReference().SetOutfit(ChildOutfit01)
		;Self.GetActorReference().EquipItem(ClothesChild01, True)
	ElseIf (clothesToEquip == ClothesChild02Male || clothesToEquip == ClothesChild02Female)
		clothesToRemove = ClothesChild02
		Self.GetActorReference().SetOutfit(ChildOutfit02)
		;Self.GetActorReference().EquipItem(ClothesChild02, True)
	ElseIf (clothesToEquip == ClothesChild03Male || clothesToEquip == ClothesChild03Female)
		clothesToRemove = ClothesChild03
		Self.GetActorReference().SetOutfit(ChildOutfit03)
		;Self.GetActorReference().EquipItem(ClothesChild03)
	ElseIf (clothesToEquip == ClothesChild04Male || clothesToEquip == ClothesChild04Female)
		clothesToRemove = ClothesChild04
		Self.GetActorReference().SetOutfit(ChildOutfit04)
		;Self.GetActorReference().EquipItem(ClothesChild04, True)
	ElseIf (clothesToEquip == ClothesChild05Male || clothesToEquip == ClothesChild05Female)
		clothesToRemove = ClothesChild05
		Self.GetActorReference().SetOutfit(ChildOutfit05)
		;Self.GetActorReference().EquipItem(ClothesChild05, True)
	Else
		;Debug.Trace("ERROR: Trying to equip clothes not in the list. " + clothesToEquip)
	EndIf
	Self.GetActorReference().RemoveItem(clothesToEquip, 1)
