...
float Property UDBPStartingHealth Auto Hidden ;UDBP 2.0.7
...
Function InitializeMiraakBattle()
	;UDBP 2.0.2 - Drop down to call the combat state change instead.
	OnCombatStateChanged( Game.GetPlayer(), 1)
...
;         debug.Trace(self + ": Initializing Script Vars")
		...
		;UDBP 2.0.7 - Set to Miraak's starting health. Used later to restore him to full power after stealing a soul so the battle won't lock up because he isn't restoring enough.
		UDBPStartingHealth = SelfActor.GetActorValue("Health")
        ...
        ActorDragon1 = Dragon1.GetActorReference()
        ActorDragon2 = Dragon2.GetActorReference()
        ActorDragon3 = Dragon3.GetActorReference()
        DLC2TameDragonOrbitMarker.moveto(SelfActor)
        SelfBase.SetEssential(False)
        SetRewardGear()
		;UDBP 2.0.2 - Disabled this script call. The function relies on AddPerk which doesn't work on NPCs, and the script being referenced is not attached to Miraak's alias.
...
;         debug.Trace(self + ": MIRAAK STEALS DRAGON SOUL")
        ...
        selfActor.RestoreActorValue("health", UDBPStartingHealth) ;UDBP 2.0.7 - Changed this because 1000 is not enough to guarantee he fully restores as intended.
...
;         debug.Trace(self + ": Dragon Kill Scene has ended")
        ...
        selfActor.RemoveSpell(DLC2MiraakFakeShoutSpell)
...
;         Debug.Trace(self + "DLC2MQ06SprintTargets length = " + listLength)
        if pickClosest
                ...
                While Distance < MinDistance
                        i = utility.RandomInt(0, DLC2MQ06SprintTargets.Length - 1) ; UDBP 2.0.2 - Bad use of list length, was overrunning the size limit.
                        NewSprintTarget = DLC2MQ06SprintTargets[i]
...
;                         debug.Trace(self + ": DragonKill waiting for landing")
                endWhile
                SelfActor.AddSpell(DLC2MiraakFakeShoutSpell)
