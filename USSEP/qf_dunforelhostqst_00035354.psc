...
alias_endSoldierActual.TryToAddToFaction(soldierFightFaction)
alias_endSoldierActual.TryToremoveFromFaction(soldierFriendlyFaction)
Alias_endSoldierActual.getActorReference().setAV("Aggression", 2.0)
alias_grifter.TryToaddToFaction(grifterFightFaction)
alias_grifter.TryToremoveFromFaction(grifterFriendlyFaction)
Alias_Grifter.getActorReference().setAV("Aggression", 2.0)

Alias_Grifter.TryToevaluatePackage()
Alias_EndSoldierActual.TryToevaluatePackage()
...
if ValmirKilled
	...
	if valmirIsImp
		...
		alias_Grifter.GetActorReference().setOutfit(SonsOutfit)
	else
		alias_EndSoldierActual.ForceRefTo(alias_EndSoldierImp.getReference())
		;alias_Grifter.getReference().additem(ImpOutfit, 1)
		alias_Grifter.GetActorReference().setOutfit(ImpOutfit)
	endif
	alias_EndSoldierActual.TryToenable()
	alias_EndSoldierActual.TryTomoveto(GrifterTurnMarkerSoldier)
	alias_Grifter.TryTomoveto(GrifterTurnMarkerValmir)
	;GrifterTurnScene.start() - USKP1.3.3 disables this here. Scene now starts from trigger outside.
...
if !valmirKilled
	alias_grifter.GetActorReference().removeItem((alias_Journal.getReference()), 1)
...
Alias_Grifter.TryToevaluatePackage()
if( alias_Grifter.GetActorReference().getActorBase() ) == ValmirImpBase
	ValmirIsImp = TRUE
elseif( alias_Grifter.GetActorReference().getActorBase() ) == ValmirSonsBase
	ValmirIsImp = FALSE
else
