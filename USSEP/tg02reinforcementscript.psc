...
;USKP 2.0.2 - Sanity checking added. Not all triggers using this are setting the same number of Mercs
Auto State Waiting
	Event OnTriggerEnter(ObjectReference triggerRef)
		Actor actorRef = triggerRef as Actor
		if(actorRef == game.getPlayer())
			if(Merc1 && Merc1.isDead() && doOnce01 == 0)
				doOnce01 = 1
				Reinforcement1.enable()
			endif
			
			if(Merc2 && Merc2.isDead() && doOnce02 == 0)
				doOnce02 = 1
				Reinforcement2.enable()
			endif
			
			if(Merc3 && Merc3.isDead() && doOnce03 == 0)
				doOnce03 = 1
				Reinforcement3.enable()
			endif
			
			if(Merc4 && Merc4.isDead() && doOnce04 == 0)
				doOnce04 = 1
				Reinforcement4.enable()
			endif
			
			if(Merc5 && Merc5.isDead() && doOnce05 == 0)
				doOnce05 = 1
