...
function MoveEntryRoomFurniture()
	...
	while currentIndex < Room01RemodelDisableList.length
		...
		;Check for NONE object added by UHFP 1.0.3
		if( currentFurniture != None )
			; if this ref is enabled, get ID from matching form list
			;UHFP 2.0.1 - Skip this step if the object in the enable array is a None. There are 3 per house.
			if( currentFurniture.IsDisabled() == false && Room01RemodelEnableList[currentIndex] != None )
			...
			if( currentFurniture.GetBaseObject() == EndTable )
				currentFurniture.RemoveAllItems(EntrywayChest)
			EndIf
			; disable this ref
...
;			foundDisableList = Room12DisableList
	...
	;UHFP 2.0.2 - Adding next bit to set ownership on the original double bed to None so the steward/bard can use it.
	;UHFP 2.0.3 - Wrong index and enable list. Had them backward. This sets the double bed in the Main Hall to allow anyone to use it.
	if( iRoomIndex == 17 && foundEnableList == Room03EnableList )
		foundEnableList[iRoomIndex].GetLinkedRef().SetFactionOwner(None)
	EndIf
...
function BuildHousePart(int iMasterIndex, int iFinishRoomID = 0, int iStartRoomID = 0)
	...
	;UHFP 2.0.1 - Added check for first index object since this gets called when disabling interior workbenches too.
	if EnableList[iMasterIndex]
...
function BuyProperty()
	...
	;UHFP 2.0.8 - Enable linked hawk ref at Lakeview
	if( SiteEnableMarker.GetLinkedRef() )
		SiteEnableMarker.GetLinkedRef().Enable()
	EndIf
	; map marker
...
;Properties added by UHFP 1.1.2 for entry way swapping
ObjectReference Property EntrywayChest Auto
Container Property EndTable Auto
