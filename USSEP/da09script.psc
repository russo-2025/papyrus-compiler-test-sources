...
;Follower and/or Animal recovery added by USKP 1.3.2
ReferenceAlias Property Follower Auto
ReferenceAlias Property AnimalFollower Auto

;USKP 2.0.1 - Bug #865: Turn off the beams.
...
function movePlayerToEarth()
	...
	if( GetStage() == 500 )
		;These two follower blocks added by USKP 1.3.2 (Bug #408) to prevent followers from becoming trapped inside the ruin once the quest is over.
		if( Follower.GetReference() != None )
			Follower.GetReference().MoveTo(PlayerRef)
		EndIf

		if( AnimalFollower.GetReference() != None )
			AnimalFollower.GetReference().MoveTo(PlayerRef)
		EndIf
