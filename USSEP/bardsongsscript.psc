...
; 	;Debug.tracestack()
	...
	;USKP 2.0.5 - OK, if the Bard is also a god damned NONE, then bail out. For Christ sake!
	if( (Bard as Actor) == None )
		Playing = 0
		Return
	EndIf

	;USKP 2.0.1 - Added check for dead bards, moved 3D check up to spare wasted cycles.
	if( (Bard as Actor).IsDead() )
		Playing = 0
		Return
	EndIf
	
	;USKP 2.0.2 - Perhaps we also need a disable check too. *shakes fist at bard spam*
	if( Bard.IsDisabled() )
		Playing = 0
		Return
	EndIf
...
; 	Debug.Trace("StopAllSongs hapenning now.")	
	...
	;USKP 2.0.6 - DB05 wedding song is not stopped with these.
...
Function StopInnMusic()
	;Modified by USKP 1.2.7 to stop errors being thrown when the player is in a NONE location.
