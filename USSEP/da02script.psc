...
; 		debug.trace("DA02Script PillarKill() start")
		...
		Victim.SetCrimeFaction(None) ;USKP 2.0.6 - Need to be removed from crime detection or the player can incur a bounty.
...
function killPreviousConduit()
	...
	if Conduit
		...
		Conduit.SetCrimeFaction(None) ;USKP 2.0.6 - Need to be removed from crime detection or the player can incur a bounty.
...
{Unpacifies, unghosts the conduit, and kills it.}
	...
	Conduit.RemoveFromFaction(WINoBodyCleanupFaction) ;USKP 2.0.1 - Conduit should be removed from this faction to prevent the cleanup script from stalling forever on the stack.
	Conduit.SetGhost(false)
	Conduit.SetCrimeFaction(None) ;USKP 2.0.6 - Need to be removed from crime detection or the player can incur a bounty.
	Conduit.kill(Game.GetPlayer())
...
; 			debug.trace("DA02Script checkDeadAndSetStage(): countCultists == 0 and Fighter1 and Fighter2 are both dead or far away. Setting Stage 500")
			...
			resurrectCounduit(DeadCultistAlias) ;added DarthVitrial for USSEP 4.3.4 Bug #24551
			setStage(17)
		EndIf
	EndIf
EndFunction
