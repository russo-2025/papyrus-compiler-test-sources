...
; 		;debug.Trace(self + " has hit " + akTarget)
		if !(akTarget as actor).hasSpell(ghostAbility) && !(akTarget as actor).IsInFaction(noHitFaction)
			...
			;USKP 2.0.5 - It was still possible for Werewolves and Vampires to contract diseases from traps even though this should not happen.
			CompanionsHousekeepingScript C00 = game.getFormFromFile(0x0004b2d9, "Skyrim.esm") as CompanionsHousekeepingScript
			PlayerVampireQuestScript PlrVampQuest = game.getFormFromFile(0x000eafd5, "Skyrim.esm") as PlayerVampireQuestScript
