...
; 				debug.Trace("Starting Dragon Combat")
			...
			gotoState("noDragon")
		Else
			;USLEEP 3.0.12 Bug #23127 - Dead dragons should not be triggering this.
			;USSEP 4.3.5 Bug #35014 - Don't try to run this code if there IS no dragon, and go to the nodragon state to stop further processing.
			if( dragonRef == None )
				goToState( "noDragon" )
			elseif( !dragonRef.IsDead() )
				if myDirtCap
					myDirtCap.Enable()
				endif
...
; 			debug.trace(self + " OnUnload: Weather.ReleaseOverride()")
	...
	;USSEP 4.1.5 Bug #16140: added this line:
	USSEP_UnregisterForAnimationEvents()
EndEvent
...
Event OnLoad()
	USSEP_RegisterForAnimationEvents()
EndEvent

function USSEP_RegisterForAnimationEvents()
	if dragonRef
		RegisterForAnimationEvent (dragonRef, "SkinFadeIn")
		RegisterForAnimationEvent (dragonRef, "UnderSkinFadeIn")
		RegisterForAnimationEvent (dragonRef, "UnderSkinGone")
	endif
endFunction

function USSEP_UnregisterForAnimationEvents()
	if dragonRef
		UnregisterForAnimationEvent (dragonRef, "SkinFadeIn")
		UnregisterForAnimationEvent (dragonRef, "UnderSkinFadeIn")
		UnregisterForAnimationEvent (dragonRef, "UnderSkinGone")
	endif
endFunction
