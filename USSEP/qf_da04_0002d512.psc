...
Game.GetPlayer().AddItem(Alias_AtunementSphere.GetReference(), 1)
Game.GetPlayer().AddItem(Alias_LexiconCubeBlank.GetReference(), 1)
...
Alias_OghmaInfinium.GetReference().BlockActivation(false)
;USKP 2.0.1 - Commented the two below. Totally unnecessary calls since we're stopping right now.
;Alias_LexiconCubeBlank.Clear()
;Alias_LexiconCubeInscribed.Clear()
Game.GetPlayer().RemovePerk(BloodHarvest) ; Added by USKP 2.0 to make sure this gets dropped.
Stop()
;END CODE
EndFunction
;END FRAGMENT
...
;USLEEP 3.0.8 Bug #21755
Alias_Extractor.GetReference().Enable()
