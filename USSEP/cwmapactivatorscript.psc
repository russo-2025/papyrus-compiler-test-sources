...
EVENT OnCellAttach()
	...
	;RemoveFlags() <--- Disabled by USKP 1.2.1. The flags will now be disabled in CWMapFlagScript.psc instead. Bug #126.

	;USSEP 4.2.9 Bug #22948 - Disable the remainder of this event. Handled via OnLoad() now.
	;if Is3DLoaded()
	;	PlaceFlags()
	;Endif
...
;USSEP 4.2.9 Bug #22948 - Should be using OnLoad since this relies on the activator being fully loaded.
Event OnLoad()
	PlaceFlags()
EndEvent
